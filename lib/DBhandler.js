...
exports.getCountsGender = function(year, callback) {
  const query = "SELECT SUM(gender='male') AS male, SUM(gender='female') AS female, SUM(gender='other') AS other FROM users WHERE state='registered'";
  queryWrapper(query, [year], (err, result) => {
...
exports.getCountsStudy = function(year, callback) {
  const query = "SELECT SUM(study='Geographie') AS Geo, SUM(study='Landschaftsökologie') AS Lök, SUM(study='Geoinformatik') AS GI, SUM(study='Zwei-Fach-Bachelor') AS ZFB FROM users WHERE state='registered'";
  queryWrapper(query, [year], (err, result) => {
    err ? callback(err) : callback(null, result[0]);
  });
}
...
exports.getCountsFood = function(year, callback) {
  const query = "SELECT SUM(food='fleischig') AS fleischig, SUM(food='vegetarisch') AS vegetarisch, SUM(food='vegan') AS vegan FROM users WHERE state='registered'";
  queryWrapper(query, [year], (err, result) => {
    err ? callback(err) : callback(null, result[0]);
  });
}
